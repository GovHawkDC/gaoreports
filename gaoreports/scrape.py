...
def search_page(start, end, page_number):
    params = {
...
def process_item(item):
    ...
    summary = item.xpath("string(.//div[contains(@class, 'c-search-result__summary')])").strip()
    ...
    links = []
    versions = product_page.xpath("//section[contains(@class, 'js-endpoint-full-report')]//a")
    ...
    primary = product_page.xpath("//div[contains(@class,'views-field-field-topic')]/div/a")
    for tag in primary:
        topics.add(tag.xpath("text()")[0].strip())

    tags = product_page.xpath("//div[contains(@class,'views-field-field-subject-term')]/div/span")
    for tag in tags:
        topics.add(tag.xpath("text()")[0].strip())
...
def scrape():
    ...
    parser.add_argument("start", type=str, help="Scrape documents published after, YYYY-mm-dd")
    parser.add_argument("end",  type=str, help="Scrape documents published before, YYYY-mm-dd")

    args = parser.parse_args()

    if not args.start:
        sys.exit("MISSING START")
